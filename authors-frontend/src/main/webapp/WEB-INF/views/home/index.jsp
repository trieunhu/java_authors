...
<c:if test="${endIndex > 1 }">
	<c:url var="nextUrl" value="/post/${currentIndex+1}" />
	<c:url var="lastUrl" value="/post/${endIndex}" />
	<c:url var="beginUrl" value="/post/${beginIndex}" />
	<c:url var="nevUrl" value="/post/${currentIndex -1}" />
	...
	<ul class="pagination justify-content-center">
		...
		<c:forEach var="i" begin="${pagination[1]}" end="${pagination[2]}">
			<c:url var="pageUrl" value="/post/${i}" />
			<c:choose>
